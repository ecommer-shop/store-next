...
// Cachear el sitemap 1 hora en producción.
// Sin esto, Next.js regenera el sitemap en cada request, lo que
// - sobrecarga la API de Vendure innecesariamente
// - hace que lastModified de las páginas estáticas cambie constantemente
export const revalidate = 3600;

const SITE_URL = (process.env.NEXT_PUBLIC_SITE_URL || 'https://ecommer.shop').replace(/\/$/, '');
const locales = ['es', 'en'];
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // 1. Rutas estáticas — lastModified fijo
  const staticPaths = ['', '/about-us', '/sellers', '/legal/terms', '/legal/privacy'];

  for (const path of staticPaths) {
    sitemapItems.push({
      url: `${SITE_URL}/${defaultLocale}${path}`,
      lastModified: new Date(STATIC_LAST_MODIFIED[path]),  // ← fecha fija
      changeFrequency: path === '' ? 'daily' : 'monthly',  // ← legal cambia poco, 'monthly' es más honesto
      priority: path === '' ? 1.0 : 0.8,
      alternates: buildAlternates(path),  // ← incluye x-default
    });
  }
