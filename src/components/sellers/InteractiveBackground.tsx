...
interface Particle {
  ...
  seed: number;
  color: string;
  baseAlpha: number; // Opacidad base individual en reposo
  angle: number;
  scale: number;
}

// Cambiamos a HEX sólidos para controlar el brillo/opacidad dinámicamente en el lienzo
const COLORS = [
  '#9969F8', // Violeta Candy Fizz
  '#6BB8FF', // Cian Blue Mana
  '#F1F1F1', // Beluga (Blanco)
];
...
export function InteractiveBackground() {
    ...
    function initParticles(w: number, h: number) {
      ...
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          ...
          // Asignamos opacidades en reposo bien tenues (Beluga es más oscuro)
          const baseAlpha = colorIndex === 2 ? 0.08 : 0.18;
    ...
    function animate() {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        if (distMouse < INFLUENCE_RADIUS && distMouse > 0) {
          ...
          // DINÁMICA DE ENCENDIDO: Sumamos un boost de brillo basado en la cercanía del cursor
          // Multiplicamos para que la transición sea exponencial y se sienta más responsiva
          currentOpacity += proximity * 5; 
        }

        // 3. Retorno elástico y fricción de fluidos
        ...
        // 4. Orientación angular al cursor
        const angleToCursor = Math.atan2(mouse.smoothY - p.y, mouse.smoothX - p.x);
        p.angle = angleToCursor + Math.PI / 1;

        // Limitar opacidad máxima para mantener el confort visual
        ...
        ctx.rotate(p.angle);
        ctx.scale(p.scale, 1);
        ctx.globalAlpha = finalOpacity; // Aplicación del canal alfa calculado
        ctx.strokeStyle = p.color;
        ctx.lineWidth = DASH_WIDTH;
        ctx.lineCap = 'round';
        ctx.beginPath();
        ctx.moveTo(-DASH_LENGTH / 2, 0);
        ctx.lineTo(DASH_LENGTH / 2, 0);
        ctx.stroke();
        ctx.restore();
      }
  ...
  return (
    <canvas
      ref={canvasRef}
      className="fixed inset-0 -z-20 bg-[#121414]"
      aria-hidden="true"
    />
  );
}