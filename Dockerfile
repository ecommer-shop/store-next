# =========================
# Builder (Bun solo para build)
# =========================
FROM oven/bun:1 AS builder
WORKDIR /app

# ---- Build-time args ----
ARG VENDURE_SHOP_API_URL
ARG NEXT_PUBLIC_VENDURE_SHOP_API_URL
ARG VENDURE_ADMIN_API_URL
ARG VENDURE_CHANNEL_TOKEN
ARG NEXT_PUBLIC_VENDURE_CHANNEL_TOKEN
ARG VENDURE_CHANNEL_TOKEN_HEADER
ARG VENDURE_AUTH_TOKEN_HEADER
ARG VENDURE_SUPERADMIN_USERNAME
ARG VENDURE_SUPERADMIN_PASSWORD
ARG VENDURE_AUTH_TOKEN_COOKIE
ARG NEXT_PUBLIC_SITE_URL
ARG REVALIDATION_SECRET
ARG NEXT_PUBLIC_SITE_NAME
ARG APP_BASE_URL
ARG AUTH0_DOMAIN
ARG AUTH0_CLIENT_ID
ARG AUTH0_CLIENT_SECRET
ARG AUTH0_API_ID
ARG AUTH0_API_IDENTIFIER
ARG USER_CLERK_ID_COOKIE
ARG CLERK_SIGN_IN_URL
ARG NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY
ARG CLERK_SECRET_KEY
ARG CLERK_WEBHOOK_SECRET
ARG CLERK_WEBHOOK_DEBUG
ARG PAYMENT_PUBLIC_KEY

# ---- Pasar args a envs ----
ENV VENDURE_SHOP_API_URL=$VENDURE_SHOP_API_URL
ENV NEXT_PUBLIC_VENDURE_SHOP_API_URL=$NEXT_PUBLIC_VENDURE_SHOP_API_URL
ENV VENDURE_ADMIN_API_URL=$VENDURE_ADMIN_API_URL
ENV VENDURE_CHANNEL_TOKEN=$VENDURE_CHANNEL_TOKEN
ENV NEXT_PUBLIC_VENDURE_CHANNEL_TOKEN=$NEXT_PUBLIC_VENDURE_CHANNEL_TOKEN
ENV VENDURE_CHANNEL_TOKEN_HEADER=$VENDURE_CHANNEL_TOKEN_HEADER
ENV VENDURE_AUTH_TOKEN_HEADER=$VENDURE_AUTH_TOKEN_HEADER
ENV VENDURE_SUPERADMIN_USERNAME=$VENDURE_SUPERADMIN_USERNAME
ENV VENDURE_SUPERADMIN_PASSWORD=$VENDURE_SUPERADMIN_PASSWORD
ENV VENDURE_AUTH_TOKEN_COOKIE=$VENDURE_AUTH_TOKEN_COOKIE
ENV NEXT_PUBLIC_SITE_URL=$NEXT_PUBLIC_SITE_URL
ENV REVALIDATION_SECRET=$REVALIDATION_SECRET
ENV NEXT_PUBLIC_SITE_NAME=$NEXT_PUBLIC_SITE_NAME
ENV APP_BASE_URL=$APP_BASE_URL
ENV AUTH0_DOMAIN=$AUTH0_DOMAIN
ENV AUTH0_CLIENT_ID=$AUTH0_CLIENT_ID
ENV AUTH0_CLIENT_SECRET=$AUTH0_CLIENT_SECRET
ENV AUTH0_API_ID=$AUTH0_API_ID
ENV AUTH0_API_IDENTIFIER=$AUTH0_API_IDENTIFIER
ENV USER_CLERK_ID_COOKIE=$USER_CLERK_ID_COOKIE
ENV CLERK_SIGN_IN_URL=$CLERK_SIGN_IN_URL
ENV NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY=$NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY
ENV CLERK_SECRET_KEY=$CLERK_SECRET_KEY
ENV CLERK_WEBHOOK_SECRET=$CLERK_WEBHOOK_SECRET
ENV CLERK_WEBHOOK_DEBUG=$CLERK_WEBHOOK_DEBUG
ENV PAYMENT_PUBLIC_KEY=$PAYMENT_PUBLIC_KEY

ENV NODE_ENV=production
ENV NEXT_TELEMETRY_DISABLED=1

COPY package.json bun.lock* ./
RUN bun install --frozen-lockfile

COPY . .
RUN bun run next build

# =========================
# Runner 
# =========================
FROM node:20-alpine AS runner
WORKDIR /app

ENV NODE_ENV=production

# Railway inyecta PORT en runtime
EXPOSE 8080

# Copiar salida standalone
COPY --from=builder /app/.next/standalone ./
COPY --from=builder /app/.next/static ./.next/static
COPY --from=builder /app/public ./public

# Script de arranque
COPY start.sh ./
RUN chmod +x start.sh

# Comando final
CMD ["./start.sh"]
